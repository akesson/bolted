...
| 02 | BoltFFI due-diligence probe (Apple) | 1 — Spike | **done** — [plan](steps/step-02-boltffi-probe.md) · [report](steps/step-02-report.md); an independent parallel probe (different wrapper design, divergent stream verdict at 0.27.3, moot at 0.27.5) lives at `crates/spike-profile-ffi-stall-probe/` |
...
| 24 | bolted-http I: the harness, the streaming verdict, the reference adapter | 4 — Harness | **ready** — [plan](steps/step-24-http-harness.md); S-CONF + S-FFI + S-LX from the spike plan: contract types (conditional `Send`, decided), the conformance suite (eleven §7 rules, generated divergence matrix, mock-first), the row-16 streaming verdict at registry 0.27.5, and the reqwest reference adapter incl. the row-19 Linux-pinning verdict. Contract freeze is a design session after the verdicts |
| 25+ | **`bolted-http` continued** (S-AP Apple adapter, S-AN Android adapter, S-WIN riding the parked C# pin) · `bolted new` (gated on the first out-of-tree framework consumer / a publishing story) · the wire emitter (D31, gated on a product feature needing the daemon topology) · `command` in core/macros (D33, with its first framework consumer) | — | sketched — see Phases 4–5 |
