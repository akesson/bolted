...
| 23 | boltffi git-pin to main: the C# resume, for real | 4 — Harness | **ready** — [plan](steps/step-23-boltffi-git-pin-csharp-resume.md); upstream #654 (C# IR backend) merged 2026-07-16, fix source-verified, unreleased → git-pin rev `23cf2ec` (Henrik, 2026-07-19); step-15 branch A executes at last (emitted C# suite, genericity, falsification); precondition for the bolted-http sequence (S-WIN.W2 rides this pin) |
| 24+ | **`bolted-http`** (v1.13 go, 2026-07-19 — sequence from `crates/bolted-http/docs/spike-plan.md` §7: S-CONF+S-FFI+S-LX, then S-AP, S-AN, S-WIN; step docs to author) · `bolted new` (gated on the first out-of-tree framework consumer / a publishing story) · the wire emitter (D31, gated on a product feature needing the daemon topology) · `command` in core/macros (D33, with its first framework consumer) | — | sketched — see Phases 4–5 |
