...
| 29 | S-WIN part I: the C# resume, at released 0.28.0 | 4 — Harness | **ready** — [plan](steps/step-29-swin-csharp-resume.md); both step-14/23 blockers verified fixed at registry 0.28.0 (MarshalAs → out-param; finding 07 → #697 distinct stream runtimes); the 9-site namespace rename, the tripwire's designed flip (red → delete, parked D23/D10/reentrant/`fillValid` probes come alive), the emitted C# contract suite (D28, 22 C-IDs) + `foreign_drift` rename + `gen-note` genericity + dotnet planted-red, kit refresh at 0.28.0. The C# **http** leg (spike-plan §5 W1/W2/W3) is NOT here — own later step, `SkipReason` verdict rides it |
