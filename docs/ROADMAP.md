...
| 25 | bolted-http II: the Apple adapter (S-AP, macOS host tier) | 4 — Harness | **in progress** — [plan](steps/step-25-apple-adapter.md); `bolted-http-apple-ffi` + `apple/bolted-http` (BoltedHttp.swift on URLSession), full conformance on the real adapter, A1 streaming probe, A6 classic-loading-mode sweep. Last implementor before the contract freeze (v1.14) |
