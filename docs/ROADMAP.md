...
| 26 | bolted-http III: the Android adapter (S-AN, instrumented ART tier) | 4 — Harness | **ready** — [plan](steps/step-26-android-adapter.md); OkHttp adapter (`BoltedHttp.kt`) + `bolted-http-android-ffi` harness bridge, opening with the N2 JNI stream probe; the last implementor before the contract freeze (v1.15 scheduling) |
| 27+ | **`bolted-http` continued** (the contract-freeze design session, then S-WIN riding the parked C# pin) · `bolted new` (gated on the first out-of-tree framework consumer / a publishing story) · the wire emitter (D31, gated on a product feature needing the daemon topology) · `command` in core/macros (D33, with its first framework consumer) | — | sketched — see Phases 4–5 |
