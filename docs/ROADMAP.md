...
| 30 | bolted-http V: the C# adapter (S-WIN part II, W1+W2) | 4 — Harness | **ready** — [plan](steps/step-30-csharp-http-adapter.md); the .NET syntheses probed (W1) then the full conformance suite through the FFI (W2) on the macOS `dotnet test` tier; `HttpClient`/`SocketsHttpHandler` foreground (WinRT rejected by the 2026-07-18 research); the `SkipReason` keep-or-delete verdict rides it; W3 (BackgroundTransfer) stays out — Windows hardware + §9-open family |
| 30+ | **harness hardening** (tier-provided sink path, row hard-kill, ALPN TestServer) · **windowed-collections design session** (agenda = step-28 report §Open questions) · `bolted new` (gated on the first out-of-tree framework consumer / a publishing story) · the wire emitter (D31, gated on a product feature needing the daemon topology) · `command` in core/macros (D33, with its first framework consumer) | — | sketched — see Phases 4–6 |
