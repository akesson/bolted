...
| 08 | bindgen evaluates no `#[cfg]` — gated items join every target's surface | ALIVE (source-verified at 0.28.0, 2026-07-21) | not filed — likely the #630/#618 cfg-eval family under RFC #665; check before duplicating | Source-read only, **runtime probe TODO** before filing or acting. Twist: `boltffi_scan::ActiveCfg` (0.28.0) is a complete, tested cfg evaluator — but unwired (dev-dep of the backend only). This is what forces the per-platform http bridge-crate split. Draft: `08-bindgen-ignores-cfg-attributes.md` |
