// A blocking newline-framed unix-socket client over raw POSIX calls — the step-18 probe client,
// made public. Deliberately primitive: request/response with pushes queued in between. Good
// enough for the extension's lazy connect-and-fetch; the interactive UI needs more (a reader
// thread demultiplexing pushes from responses) — see WireConnection (M2), and the friction log:
// the split between "probe client" and "UI client" is itself generator evidence.

import Foundation

public enum ConnectOutcome {
    case connected(LineClient)
    /// connect(2) failed; the errno is the evidence (the G3 control expects EPERM/ENOENT under
    /// the extension sandbox outside the group container).
...
public final class LineClient {
    ...
    public static func connect(path: String, timeoutSeconds: Int = 10) -> ConnectOutcome {
    ...
    public func request(_ req: Request) -> Resp? {
        seq += 1
