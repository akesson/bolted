//! `bolted-http-android-ffi` — the Android **harness bridge** for the `bolted-http` capability
//! contract (step 26, milestone M0).
//!
//! This crate is the effect-side analog of the generated capability glue: the [`Http`] capability
//! crosses the FFI as a BoltFFI **callback trait** ([`HttpAdapter`]) that a hand-written Kotlin
//! OkHttp adapter implements. It mirrors `bolted-http-apple-ffi` exactly — the FFI surface is the
//! decided topology; only the doc language and the packaged target differ. On the Rust side it
//! exposes three things to the instrumented ART tier (a Gradle-managed-device conformance suite):
//!
//! 1. a **conformance driver** ([`HttpHarness::run_c1`]) that runs the real `bolted-http`
//!    conformance rows against the registered Kotlin adapter and returns **structured** per-row
...
//! 3. the **completion re-entry** points the Kotlin adapter calls back through
...
//! `drive_*` helpers. Our [`AdapterFactory`] yields a [`KotlinAdapter`] shim whose
...
//! (d) calls the Kotlin adapter's `execute`. `execute` returns immediately (OkHttp dispatches the
//! `Call` asynchronously); the completion arrives later on an OkHttp dispatcher thread and re-enters
//! through [`HttpHarness::complete_ok`] / [`HttpHarness::complete_err`], which look the token up,
//! convert back to the contract types, and deliver to the parked sink — unblocking the row.
//!
//! M0 scope: the walking-skeleton Kotlin adapter honours exactly one C1 row (rule 1 — GET `/ok`).
...
// reads these as SOURCE TEXT and emits the Kotlin structs/enums; the rich `bolted-http` types
...
/// One SHA-256-of-SPKI pin the request carries (32 bytes; feature-matrix §5.14, rule 10). The Kotlin
...
/// request carries the *data* regardless (defaulting to [`FfiPriority::Normal`] when unset), but the
/// OkHttp adapter **legally ignores** it — OkHttp exposes no per-`Call` priority knob, so the Android
/// adapter does not honour the hint and `PriorityHint` is absent from its C3 column (the decided
/// Apple/Android divergence). Mirrors [`Priority`].
#[data]
#[derive(Clone, Copy)]
...
pub struct FfiRequest {
    ...
    /// The request's priority hint (row 12, CAP). The OkHttp adapter ignores it (no priority knob);
    /// the data still rides every request. Defaults to [`FfiPriority::Normal`] when the request set none.
    pub priority: FfiPriority,
}

/// The negotiated HTTP version, mirrored across the boundary (feature-matrix row 11). M1 drops the
/// M0 `Http1_1` placeholder: the Kotlin adapter reads the real protocol from OkHttp `Response.protocol`
/// and reports it here.
#[data]
#[derive(Clone, Copy)]
...
pub struct FfiResponse {
    ...
    /// The negotiated HTTP version, read from OkHttp `Response.protocol` (row 11). No longer a
    /// placeholder.
...
/// The typed error keys the Kotlin adapter maps native failures to. M1 covers the full C2 taxonomy
/// the OkHttp adapter can reach on the host tier; each maps to a [`HttpError`] variant so the
...
pub enum FfiHttpError {
    /// The deadline elapsed (synthesized total-deadline timer, or an OkHttp `SocketTimeoutException`).
    Timeout,
    /// The caller cancelled the in-flight effect (an OkHttp `IOException("Canceled")`, not deadline-caused).
    Cancelled,
    ...
    /// positive control, M2).
    Io,
    /// The OS refused permission for the request (a missing `INTERNET` permission surfacing as a
    /// `SecurityException`, or a sandbox network denial). Distinct from a network failure (§5.15).
    /// Platform-gated on the ART tier — see the M2 notes (feeds freeze Q5); the adapter maps a
    /// genuine OS permission denial here, never invents the key.
    PermissionDenied,
    /// The redirect chain exceeded the limit (OkHttp's follow-up-count cap). `limit` is the
    /// ceiling that fired; OkHttp enforces its own follow-up cap in M1 (the request carries no
...
/// The three test-server base URLs handed to Kotlin on [`HttpHarness::start_server`], plus the TLS
...
/// [`FailureReason`] (or skip reason) so a Kotlin test can print *why* a row went red.
#[data]
...
/// Mirrors the step-24 S-FFI `Chunk`. `t_send_ns` is stamped by Kotlin
/// (`System.nanoTime()`) immediately before the deliver call so the consumer can
...
// =====================================================================================
// The callback trait Kotlin implements.
// =====================================================================================

/// The HTTP capability as it crosses the FFI: the Kotlin OkHttp adapter implements `execute`.
...
pub trait HttpAdapter: Send + Sync {
    /// Dispatch a request effect. Must return promptly (OkHttp `Call.enqueue` is non-blocking);
    ...
    /// adapter cancels the OkHttp `Call`, which completes with an OkHttp `IOException("Canceled")`, which the
    /// adapter maps to [`FfiHttpError::Cancelled`]). A no-op if the token is unknown / already done.
    fn cancel(&self, token: u64);
}

// =====================================================================================
// Shared bridge state + the `Http` shim that fronts the Kotlin adapter for the suite.
// =====================================================================================

/// A parked row completion, keyed by token until the Kotlin adapter delivers.
struct Pending {
    completion: Box<dyn CompletionSink>,
    /// The row's upload-progress sink, if any (rule 11). The Kotlin adapter's `didSendBodyData`
    /// delegate re-enters [`HttpHarness::report_progress`], which forwards to this sink.
    progress: Option<Box<dyn UploadProgressSink>>,
}

/// State shared between the `Http` shim (which needs the Kotlin adapter + the token registry) and
...
/// registered Kotlin adapter (the OkHttp client is stateless per request, so every row shares it).
struct KotlinAdapter {
    shared: Arc<Shared>,
}

impl Http for KotlinAdapter {
    fn send(
        &self,
        request: HttpRequest,
        completion: Box<dyn CompletionSink>,
        ...
        // Rust → Kotlin: performs the request asynchronously and returns immediately.
        self.shared.adapter.execute(ffi);

        // Bridge the contract's poll-based cancellation to the Kotlin task: a detached watcher polls
        // the returned token (the Linux adapter's 10 ms poll, mirrored) and, on cancellation,
        // forwards `adapter.cancel(token)` across the FFI so the OkHttp `Call` is cancelled. The
...
// Row 12 (CAP): NO `impl PriorityHint for KotlinAdapter`. Decided (step 26): the priority hint is a
// CAP the OkHttp path **legally ignores** — OkHttp exposes no per-`Call` priority knob, so the
// Android adapter does not honour it. The hint *data* still rides every request (`FfiRequest.priority`),
// but the capability is ABSENT: [`KotlinFactory::priority_hint`] returns `None`, so the C3 Android
// column reports `priority-hint absent` — the honest Apple/Android divergence (Apple maps it to
// `URLSessionTask.priority`; Android cannot). This is the row-12 divergence the C3 table exists to show.

/// Row 18 (CAP, tiered): OkHttp exposes per-phase timings via an `EventListener`
/// (dnsStart/connectStart/secureConnectStart/responseHeadersStart — DNS/connect/TLS/first-byte), so
/// the honest tier is [`MetricsTier::Phase`] — the same tier as the Apple adapter, richer than
/// reqwest's whole-request tier. The C3 Android column reads this off the trait impl.
impl Metrics for KotlinAdapter {
...
struct KotlinFactory {
    shared: Arc<Shared>,
}

impl AdapterFactory for KotlinFactory {
    ...
    // NB: no `priority_hint` override — OkHttp legally ignores the hint (row 12, CAP), so the factory
    // falls through to the trait default (`None`) and the C3 Android column reports `priority-hint
    // absent`. The divergence from Apple (which returns `Some`) is exactly what the C3 table records.

    /// Present at the [`MetricsTier::Phase`] tier (row 18, CAP tiered) — OkHttp `EventListener`.
    fn metrics(&self) -> Option<Box<dyn Metrics>> {
        Some(Box::new(KotlinAdapter {
            shared: Arc::clone(&self.shared),
        }))
    }
}
...
/// The Rust half of the bridge Kotlin drives. Constructed with the Kotlin adapter; owns the shared
/// registry and (once started) the in-process test server.
pub struct HttpHarness {
    shared: Arc<Shared>,
    server: Mutex<Option<(TestServer, Endpoints)>>,
    /// A1 streaming probe (step 25): the `ffi_stream` a live Kotlin consumer drains. Chunks pushed by
...
#[export]
impl HttpHarness {
    /// Build the harness over the registered Kotlin adapter (the composition-root dance: adapter
    /// first, harness second, then the Kotlin side sets its weak back-reference to this harness).
    ...
    /// Upload-progress re-entry (rule 11): forward the Kotlin adapter's `didSendBodyData` figures to
    ...
    /// not known up front (a -1 unknown content length).
    ...
    /// Run the eleven C1 conformance rows against the registered Kotlin adapter (structured results).
    ...
    /// The C3 divergence table for the Android adapter, rendered from the capability traits (row 12
    ...
    // inside a real http round-trip on the Android path: the Kotlin OkHttp streaming consumer
    // reads the test server's `/chunked` endpoint and pushes each chunk here via `deliver_chunk`;
    // a live Kotlin consumer drains `chunk_stream` and proves ordered/lossless/complete delivery.
    // NO contract surface is added — the streaming core seam is deliberately unfrozen (freeze Q2).

    /// A1 deliver: a response-body chunk crossing the FFI from the Kotlin streaming consumer re-enters
    ...
    /// A1: the live response-stream the Kotlin consumer drains as an `Flow<Chunk>` (F1 —
...
impl HttpHarness {
    ...
    /// Drive `rows` against the registered Kotlin adapter over the started server, projecting each
...
fn to_http_response(response: &FfiResponse) -> Result<HttpResponse, HttpError> {
    ...
    // (§5.12). The version is the adapter's real OkHttp `EventListener` / `Response.protocol` observable (row 11).
